...
=> – This is a short-hand (expression-bodied property) for a read-only property; it directly returns the expression result.
                ...
                Func<MeasurementsData, Task> onDataReceived)
        {
            ...
            /*------ COMME NodesToMonitor n’existe plus ; pas de notion de node / variable ; pas compatible JSON final
            ...
            // ---  BOUCLE DES NOEUDS À MONITORER ------ 

            foreach (var node in plc.Nodes)
            {
                foreach (var variable in node.Variables.Where(v => v.Enabled))
                {
                    var item = new MonitoredItem(_subscription.DefaultItem)
                    {
                        ...
                        QueueSize = 10,
                        DiscardOldest = true,
                        DisplayName = $"{plc.Name}.{node.Name}.{variable.Name}"
                    };


                    // Capture context 
                    var plcName = plc.Name;
                    ...
                    Console.WriteLine(
                        $"[{plc.Name}] Monitoring {node.Name}.{variable.Name}");
                }
            }
        ...
        // ── Handlers ─────
        /* ------- Plus utile, car : pas de NodeName ; pas de VariableName ; logique déplacée inline (plus clair, moins de capture)
